...
def resize(W, H, ver):
    xMax,yMax,xMin,yMin = ver[0][0],ver[0][1],ver[0][0],ver[0][1]
    for i in ver:
    ...
    coefX=int((W-10)/absX)
    coefY=int((H-10)/absY)
    coefAll=min(coefX,coefY)
    
    cenX=int(coefAll*(xMax+xMin)/2)
    cenY=int(coefAll*(yMax+yMin)/2)
    cenW=W/2
    cenH=H/2
    ...
    for i in ver:
        i[0],i[1],i[2] = i[0]*coefAll+pointsToCenterW,i[1]*coefAll+pointsToCenterH,i[2]*coefAll
